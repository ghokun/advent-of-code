...
-- part 2
WITH calibration_values AS (
  SELECT ARRAY(SELECT ARRAY_TO_STRING(REGEXP_MATCHES(calibration, '((?=\d)\w{1}|(?=(one|two|three|four|five|six|seven|eight|nine))\w{2})', 'g'), '')) AS digit, calibration
    FROM day01.input
)
SELECT SUM(CAST((CASE digit[1]
          WHEN 'on' THEN '1'
          WHEN 'tw' THEN '2'
          WHEN 'th' THEN '3'
          WHEN 'fo' THEN '4'
